...
const statusLabel = (value: string) => value === 'pending' ? 'Order Received' : value === 'completed' ? 'Completed' : readable(value)
...
export function OrderDetails({ orderId }: { orderId: string }) {
  ...
  async function update(field: 'payment_status' | 'order_status', value: string) {
    ...
    try {
      ...
      setFeedback(field === 'order_status' ? `Order status changed to ${statusLabel(value)}.` : 'Payment status updated.')
      await load()
  ...
  async function saveTracking() {
    ...
    try {
      await requireAdminSession()
      const cleanCourier = courier.trim() || null
      const cleanTracking = trackingNumber.trim() || null
      const { error: updateError } = await supabase.from('orders').update({ courier: cleanCourier, tracking_number: cleanTracking, updated_at: new Date().toISOString() }).eq('id', order.id)
      if (updateError) throw updateError
      setFeedback(cleanTracking ? 'Tracking details saved.' : 'Tracking details cleared.')
      await load()
    } catch (caught) { setError(caught instanceof Error ? caught.message : 'Tracking details could not be saved.') }
  }
