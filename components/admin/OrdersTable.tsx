...
export function OrdersTable() {
  ...
  async function load() {
    const { data, error: loadError } = await supabase
      .from('orders')
      .select('id,order_reference,customer_name,mobile_number,delivery_method,payment_method,selected_payment_option_name,upfront_amount,rider_collectible_amount,showroom_payable_amount,payment_status,order_status,payment_proof_path,telegram_notification_status,created_at,order_notes')
  ...
  async function proof(path: string) {
    ...
    if (proofError) return setError(proofError.message)
    ...
    <div className={styles.tableWrap}><table className={styles.table}>
      ...
      <tbody>{orders.map((order) => <tr key={order.id}>
        ...
        <td>
          ...
          {order.telegram_notification_status === 'failed' && <button className={`${styles.tableAction} ${styles.retryNotificationAction}`} type="button" disabled={retryingOrderId === order.id} onClick={() => void retryTelegram(order)}>{retryingOrderId === order.id ? 'Retrying…' : 'Retry Telegram'}</button>}
        </td>
