...
function timeline(order: Order) {
  ...
  return [
    ...
    ...['Preparing Order', 'Packed', order.delivery_method === 'same_day_delivery' ? 'Ready for Rider' : order.delivery_method === 'showroom_pickup' ? 'Ready for Pickup' : 'Shipped', 'Delivered'].map((name, index) => ({ name, state: paymentVerified && currentOrderStage >= index + 2 ? 'completed' : paymentVerified && currentOrderStage === index + 1 ? 'current' : 'upcoming' })),
  ]
}
