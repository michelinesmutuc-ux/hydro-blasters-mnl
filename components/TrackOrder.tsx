...
const statusLabel = (value: string) => value === 'pending' ? 'Order Received' : value === 'completed' ? 'Completed' : label(value)
const maskMobile = (value: string) => value.length > 4 ? `${value.slice(0, 4)}•••${value.slice(-4)}` : value

function timeline(order: Order) {
  if (order.order_status === 'cancelled') return [{ name: 'Order Cancelled', state: 'current' }]
  const isSameDay = order.delivery_method === 'same_day_delivery'
  const stages = isSameDay
    ? [
        { value: 'pending', name: 'Order Received' },
        { value: 'preparing', name: 'Preparing' },
        { value: 'packed', name: 'Packed' },
        { value: 'ready_for_rider', name: 'Ready for Rider' },
    ...
    : [
        ...
        { value: 'preparing', name: 'Preparing' },
        { value: 'packed', name: 'Packed' },
        { value: 'shipped', name: order.delivery_method === 'showroom_pickup' ? 'Ready for Pickup' : 'Shipped' },
        { value: 'completed', name: 'Completed' },
      ]
  const legacyMap: Record<string, string> = { reservation_pending: 'pending', confirmed: 'pending', delivered: 'completed' }
  const currentValue = legacyMap[order.order_status] || order.order_status
  const currentIndex = Math.max(stages.findIndex((stage) => stage.value === currentValue), 0)
  return stages.map((stage, index) => ({ name: stage.name, state: index < currentIndex ? 'completed' : index === currentIndex ? 'current' : 'upcoming' }))
}
