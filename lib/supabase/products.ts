...
export const publicProductColumns = 'id,name,slug,brand,category,product_type,price,stock,status,image_urls,shipping_class,is_clearance,has_variants,variant_group_name,featured,show_on_homepage,highlight_type,homepage_sort_order,created_at,short_description'
export const adminProductColumns = 'id,name,slug,brand,category,product_type,price,stock,status,shipping_class,is_clearance,has_variants,variant_group_name,short_description,description,specifications,image_urls,featured,is_active,show_on_homepage,highlight_type,homepage_sort_order,created_at,updated_at'
...
export async function fetchActiveProductBySlug(slug: string) {
  return supabase
    .from('products')
    .select('id,name,slug,brand,category,product_type,price,stock,status,short_description,description,image_urls,shipping_class,is_clearance,has_variants,variant_group_name')
    .eq('slug', slug)
    .eq('is_active', true)
    .maybeSingle()
}
...
export async function fetchAdminProduct(productId: string) {
  return supabase
    .from('products')
    .select('id,name,slug,brand,category,product_type,price,stock,status,shipping_class,is_clearance,has_variants,variant_group_name,short_description,description,featured,is_active,show_on_homepage,highlight_type,homepage_sort_order,image_urls')
    .eq('id', productId)
    .single()
}
